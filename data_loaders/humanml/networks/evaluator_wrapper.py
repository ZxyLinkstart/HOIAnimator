...
def build_models(opt):
    ...
    checkpoint = torch.load(pjoin(opt.checkpoints_dir, opt.dataset_name, 'text_mot_match', 'model', 'E0130.tar'),
...
def build_evaluators(opt):
    ...
    checkpoint = torch.load(pjoin("/root/zxy/InterHDM/motion-diffusion-model/eval/", ckpt_dir, 'text_mot_match', 'model', 'E0065.tar'),
                            map_location=opt['device'])
    movement_enc.load_state_dict(checkpoint['movement_encoder'])
    text_enc.load_state_dict(checkpoint['text_encoder'])
    motion_enc.load_state_dict(checkpoint['motion_encoder'])
...
class EvaluatorMDMWrapper(object):
    ...
    def __init__(self, dataset_name, device):
        opt = {
            ...
            'dim_pose': 165 if dataset_name == 'humanml' else 251,
