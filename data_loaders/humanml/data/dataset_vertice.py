...
class Text2MotionDatasetV2(data.Dataset):
    def __init__(self, opt, mean, std, split_file, w_vectorizer):
        ...
        for id in range(len(behave_data['poses'])):
            text_dict = {}
            motion = behave_data['poses'][id][:,:165].numpy()
            ...
            if f_tag == 0.0 and to_tag == 0.0:
                ...
                try:
                    ...
                    # break
            if flag:
                data_dict[id] = {'motion': motion,
                                    'length': len(motion),
                                    'text': text_data,
                                    'motion_lable':behave_data['motion_label'][id],
                                    'obj_lable':behave_data['obj_label'][id],
                                    'gt_obj_mesh':behave_data['obj_mesh'][id]}
                
                new_name_list.append(id)
                length_list.append(len(motion))
    ...
    def __getitem__(self, item):
        ...
        if m_length < self.max_motion_length:
            ...
            gt_mesh = np.concatenate([gt_mesh,
                                     np.zeros((self.max_motion_length - m_length, gt_mesh.shape[1], gt_mesh.shape[2]))
                                     ], axis=0)
        # print(word_embeddings.shape, motion.shape)
        # print(tokens)
        return word_embeddings, pos_one_hots, caption, sent_len, motion, m_length, '_'.join(tokens),gt_mesh,motion_lable,obj_lable
