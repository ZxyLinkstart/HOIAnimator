...
obj_path = "/root/zxy/motion-diffusion-model/objects"
simplified_mesh = {
    0:"backpack/backpack_f1000.ply",
    1:"basketball/basketball_f1000.ply",
    2:"boxlarge/boxlarge_f1000.ply",
    3:"boxtiny/boxtiny_f1000.ply",
    4:"boxlong/boxlong_f1000.ply",
    5:"boxsmall/boxsmall_f1000.ply",
    6:"boxmedium/boxmedium_f1000.ply",
    7: "chairblack/chairblack_f2500.ply",
    8: "chairwood/chairwood_f2500.ply",
    9: "monitor/monitor_closed_f1000.ply",
    10:"keyboard/keyboard_f1000.ply",
    11:"plasticcontainer/plasticcontainer_f1000.ply",
    12:"stool/stool_f1000.ply",
    13:"tablesquare/tablesquare_f2000.ply",
    14:"toolbox/toolbox_f1000.ply",
    15:"suitcase/suitcase_f1000.ply",
    16:"tablesmall/tablesmall_f1000.ply",
    17: "yogamat/yogamat_f1000.ply",
    18:"yogaball/yogaball_f1000.ply",
    19:"trashbin/trashbin_f1000.ply",
}
...
class Text2MotionDatasetV2(data.Dataset):
    def __init__(self, opt, mean, std, split_file, w_vectorizer):
        ...
        # mesh_list = []
        # for i in range(20):
        #     mesh_path = os.path.join(obj_path,simplified_mesh[i])
        #     temp_simp = Mesh()
        #     temp_simp.load_from_file(mesh_path)
        #     mesh_list.append(temp_simp)
        ...
        behave_data = joblib.load("/root/zxy/motion-diffusion-model/dataset/BHEAVE_HOI.pkl")
        for index in range(10000):
            
            id = index % len(behave_data['poses'])
            text_dict = {}
            motion = behave_data['poses'][id][:60,:165].numpy()
            text_dict['caption'] = behave_data['text'][id]
            tokens = behave_data['caption'][id].split('#')[1].split(' ')
            f_tag = float(behave_data['caption'][id].split('#')[2])
            to_tag = float(behave_data['caption'][id].split('#')[3])
            text_data = []
            ...
            if f_tag == 0.0 and to_tag == 0.0:
                ...
                try:
                    ...
                    print(behave_data['caption'][id])
                    # print(behave_data['caption'][id]plit[2], behave_data['caption'][id][3], f_tag, to_tag, name)
                    # break
            if flag:
                data_dict[index] = {'motion': motion,
                                    'length': len(motion),
                                    'text': text_data,
                                    'motion_lable':behave_data['motion_label'][id],
                                    'obj_lable':behave_data['obj_label'][id],
                                    }
                
                new_name_list.append(index)
                length_list.append(len(motion))
    ...
    def __getitem__(self, item):
        ...
        motion_lable = data['motion_lable']
        obj_lable = data['obj_lable']
        ...
        # print(tokens)
        return word_embeddings, pos_one_hots, caption, sent_len, motion, m_length, '_'.join(tokens),motion_lable,obj_lable
...
class TextOnlyDataset(data.Dataset):
    def __init__(self, opt, mean, std, split_file, w_vectorizer):
        ...
        behave_data = joblib.load("/root/zxy/motion-diffusion-model/dataset/BHEAVE_HOI.pkl")
        for index in range(len(behave_data['poses'])):
        ...
        # for id in range(32):    
            ...
            if f_tag == 0.0 and to_tag == 0.0:
                ...
                try:
                    ...
                    # break
            if flag:
                data_dict[index] = {'text': text_data}
                new_name_list.append(index)    
    ...
    def __getitem__(self, item):
        ...
        # print(word_embeddings.shape, motion.shape)
        # print(tokens)
        return None, None, caption, None, np.array([0]), self.fixed_length, '_'.join(tokens),None,None
        # return word_embeddings, pos_one_hots, caption, sent_len, motion, m_length, '_'.join(tokens),motion_lable,obj_lable
    # def __getitem__(self, item):
    #     idx = self.pointer + item
    #     data = self.data_dict[self.name_list[idx]]
...
class HumanML3D(data.Dataset):
    def __init__(self, mode, datapath='./dataset/humanml_opt.txt', split="train", **kwargs):
        ...
        if mode == 'text_only':
            ...
            self.t2m_dataset = TextOnlyDataset(self.opt, self.mean, self.std, self.split_file, self.w_vectorizer)
        else:
            self.mean = None
            self.std = None
            self.w_vectorizer = WordVectorizer(pjoin(abs_base_path, 'glove'), 'our_vab')
