...
def evaluate_matching_score(eval_wrapper, motion_loaders, file):
    ...
    for motion_loader_name, motion_loader in motion_loaders.items():
        ...
        # word_embeddings, pos_one_hots, caption, sent_len, motion, m_length, '_'.join(tokens),motion_lable,obj_lable
        with torch.no_grad():
            for idx, batch in enumerate(motion_loader):
                word_embeddings, pos_one_hots, _, sent_lens, motions, m_lens, _ = batch[:7]
...
def evaluate_fid(eval_wrapper, groundtruth_loader, activation_dict, file):
    ...
    with torch.no_grad():
        for idx, batch in enumerate(groundtruth_loader):
            _, _, _, sent_lens, motions, m_lens, _ =  batch[:7]
...
if __name__ == '__main__':
    ...
    gen_loader = get_dataset_loader(name=args.dataset, batch_size=args.batch_size, num_frames=None, split=split, hml_mode='text_only')
