...
class TrainLoop:
    def __init__(self, args, train_platform, model, diffusion, data):
        ...
        state_dict = dist_util.load_state_dict(
                '/root/zxy/InterHDM/motion-diffusion-model/save/vertice_final_1_1/opt000600000.pt', map_location=dist_util.dev()
            )
        self.opt.load_state_dict(state_dict)
        
        # param_groups = torch.load('/root/zxy/InterHDM/motion-diffusion-model/save/ori_0728/opt000600000.pt')['param_groups'][0]
        
        
        # for k in param_groups:
        #     param_groups[k] = torch.tensor(param_groups[k]).cuda()
        # self.opt.add_param_group(param_groups)
        
        # state = torch.load('/root/zxy/InterHDM/motion-diffusion-model/save/ori_0728/opt000600000.pt')['state'][0]
        # for k in state:
        #     if torch.is_tensor(state[k]):
        #         state[k].cuda()
        #     else:
        #         state[k] = torch.tensor(state[k]).cuda()
        
        # self.opt.load_state_dict(state)
        
        
        # # 加载优化器状态字典
        # checkpoint = torch.load('/root/zxy/InterHDM/motion-diffusion-model/save/ori_0728/opt000600000.pt')
        # for i, param_group in enumerate(checkpoint['param_groups']):
        #     if i >= len(self.opt.param_groups):
        #         break
        #     self.opt.param_groups[i]['lr'] = param_group['lr']
        #     self.opt.param_groups[i]['weight_decay'] = param_group['weight_decay']  
        if self.resume_step:
