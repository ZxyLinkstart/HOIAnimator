...
class GaussianDiffusion:
    ...
    def training_losses(self, model, x_start, t, model_kwargs=None, noise=None, dataset=None):
        ...
        if hasattr(model.model, "is_multi") and model.model.is_multi:
            ...
            model_kwargs['y']['other_motion'] = self.q_sample(x_start, t, noise=noise2, model_kwargs=model_kwargs)
        ...
        if self.loss_type == LossType.KL or self.loss_type == LossType.RESCALED_KL:
            ...
            terms["rot_mse"] = self.masked_l2(target[:,:159,:,:], model_output[:,:159,:,:], mask) # mean_flat(rot_mse)
            
            print("=============",terms["rot_mse"],"================")
            
            
            # TODO FIX vertices loss    
            
            if self.vertice > 0:
                terms["vertice"] = self.masked_l2(target[:,159:165,:,:], model_output[:,159:165,:,:], mask)
                print("======terms['vertice'] =======",terms["vertice"],"================")
                # get obj
                # mesh_list = dataset.t2m_dataset.mesh_list
                # obj_lable =  model_kwargs['y']['obj_lable']
                
                
                
                # mesh_output = []
                # batch_size = x_start.shape[0]
                # angle = target.permute(0,3,1,2)[:,:,159:162,0]
                # trans = target.permute(0,3,1,2)[:,:,162:165,0]
                
                # labeles = model_kwargs['y']['obj_lable']
                # vertice_loss_all = []
                
                
                # pre_angle = model_output.permute(0,3,1,2)[:,:,159:162,0]
                # pre_trans = model_output.permute(0,3,1,2)[:,:,162:165,0]
                
                
                # for i in range(batch_size):
                #     lengths = model_kwargs['y']['lengths'][i].item()
                #     # Convert rotation vectors to anti-symmetric matrices
                #     # rot = torch.tensor(Rotation.from_rotvec(angle.cpu().detach().numpy()[i]).as_matrix(),).cuda()
                #     rot = geometry.axis_angle_to_matrix(angle[i])
                #     pre_rot = geometry.axis_angle_to_matrix(pre_angle[i])
                #     # rot = tgm.rotation_matrix_3d(angle[i])

                #     tran = trans[i]
                #     pre_tran = pre_trans[i]
                ...
                #     # center!!!!!!
                #     mesh =  mesh_list[label].v
                    
                #     center = np.mean(mesh_list[label].v,0)
                    
                #     mesh -= center
                    
                #     mesh = np.tile(mesh[np.newaxis, :, :], (len(rot), 1, 1))
                #     mesh = torch.tensor(mesh).cuda().to(torch.float64)
                    
                #     rot = rot.to(torch.float64)
                #     pre_rot = pre_rot.to(torch.float64)
                    
                    
                #     tran = tran.to(torch.float64)
                #     pre_tran = pre_tran.to(torch.float64)
                    
                #     mesh = torch.matmul(mesh, torch.transpose(rot, 1, 2)) + tran[:, None, :]
                #     pre_mesh = torch.matmul(mesh, torch.transpose(pre_rot, 1, 2)) + pre_tran[:, None, :]
                #     # pre_mesh = np.matmul(mesh, np.transpose(rot, (0, 2, 1))) + np.array(tran)[:, np.newaxis, :]

                #     loss = torch.sum(self.l2_loss(mesh[:lengths,:,:], pre_mesh[:lengths,:,:]))/mesh.shape[1] * 0.0001
                #     # print('mse_loss_val', mse_loss_val)
                #     vertice_loss_all.append(loss)
                #     # mesh_output.append(pre_mesh)
                #     # erro = torch.sum(pre_mesh[:lengths,:,:]-tar_mesh)
                #     # erro = torch.sum(self.l2_loss(pre_mesh[:lengths,:,:],tar_mesh))
                #     # vertice_loss_all.append(erro)
                
                # terms['vertice'] = torch.vstack(vertice_loss_all)
                
                
                # for id in range(len(obj_lable)):
                #     lengths = model_kwargs['y']['lengths'][id].item()
                #     pre_mesh =  mesh_list[obj_lable[id]]
                #     center = np.mean(pre_mesh.v, 0)
                #     pre_mesh.v -= center
                    
                #     # model_output = x_start
                #     angle = model_output[id][159:162].permute(1,2,0)[0].cpu()[:lengths,:]
                #     trans_obj = model_output[id][162:165].permute(2,1,0).cpu()[:lengths,:]
                #     rot = Rotation.from_rotvec(angle.detach().numpy()).as_matrix()
                    
                #     temp_simp_transformed = Mesh(pre_mesh.v.copy(), pre_mesh.f.copy())
                #     temp_list = np.tile(pre_mesh.v, (lengths, 1, 1))
                    
                #     out_vertice = np.matmul(temp_simp_transformed.v, rot) + np.array(trans_obj.detach().numpy())

                #     targe_vertice = model_kwargs['y']['gt_mesh'][id][:lengths,:,:]
                    
                    
                #     erro = self.l2_loss(out_vertice,targe_vertice)
                
                
                # label = 1
                # # obj_fit = Object(angle.cpu(),trans_obj.cpu(),label)
            ...
            terms["loss"] = terms["rot_mse"] + terms.get('vb', 0.) +\
                            ...
                            (self.vertice * terms.get('vertice', 0.))
